...
def load_deer_observed_csv(
    ...
    """Load observed deer series from a CSV file.

    Inputs:
      - path: CSV file path
      - year_field: header for the year column
      - value_field: header for the deer value column (numbers can include commas)
      - interpolate_missing: if True, fill missing years by linear interpolation

    Output:
      - (years, values, imputed_flags): lists of equal length; imputed_flags[i] is True if value was filled
    """
    raw_years: List[int] = []
    raw_vals: List[float] = []
    ...
    # Linear interpolation for missing integer years between min and max
    years: List[int] = []
    values: List[float] = []
    imputed: List[bool] = []
